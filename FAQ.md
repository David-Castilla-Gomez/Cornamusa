...
Es funcional para programas reales: OOP completo con dunders, closures con `nolocal`, pattern matching, generadores, comprehensions, GC, excepciones con traceback, módulos y una stdlib de diecisiete módulos (`archivos`, `json`, `csv`, `base64`, `hashing`, `jwt`, `regex`, `fechas`, `tiempo`, `azar`, `proceso`, `red`...). Sirve bien para **enseñar a programar en castellano** y para scripting pequeño/mediano.
