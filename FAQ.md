...
Es funcional para programas reales: OOP completo con dunders, closures con `nolocal`, pattern matching, generadores, comprehensions, GC, excepciones con traceback, módulos y una stdlib de veintitrés módulos (`archivos`, `json`, `csv`, `base64`, `hashing`, `jwt`, `regex`, `fechas`, `tiempo`, `azar`, `proceso`, `red`, `coleccion`, `inspeccion`, `validacion`, `argumentos`, `ruta`, `pruebas`...). Sirve bien para **enseñar a programar en castellano** y para scripting pequeño/mediano.
...
- Pendiente del lado de tooling: integración con editores que enganche el depurador (hoy es CLI). El subcomando `cornamusa depurar` ya está, igual que `fmt`, `lint`, `docs`, `lsp`, `prof` y `cov` — ver detalle más abajo.
...
| v1.47-v1.55 — REPL line-editing, `fmt`, `lint` (eventualmente 16 categorías), `docs`, LSP | ✅ |
